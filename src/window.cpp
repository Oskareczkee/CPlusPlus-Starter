...
void Window::InitModels(){
    ...
    shapes.emplace_back((new FilledStar(5, 0.3, 0.8))->SetFillColorRGB(color));
    shapes.emplace_back((new Star(5, 0.3, 0.8))->SetFillColorRGB(color));
    shapes.emplace_back((new FilledCircle(0.5, 30))->SetFillColorRGB(color));
    shapes.emplace_back((new Circle(0.5, 30))->SetFillColorRGB(color));
    shapes.emplace_back((new FancyStar(5, 0.3, 0.8))
                             ->SetFrameColor(color)
                             ->SetFillColor(color.SetSaturation(0.2)));
...
void Window::Run(void){
    while (!glfwWindowShouldClose(window_)){
        ...
        shapes[current_shape_idx]->Draw(program_);
